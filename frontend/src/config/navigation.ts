...
export const navigationConfig: MenuItem[] = [
  { path: '/dashboard', label: '대시보드', icon: Home, roles: ['ADMIN', 'DISPATCHER'], mobileVisible: true },
  { path: '/orders', label: '주문 관리', icon: Package, roles: ['ADMIN', 'DISPATCHER'], mobileVisible: true },
  { path: '/calendar', label: '오더 캘린더', icon: Calendar, roles: ['ADMIN', 'DISPATCHER'] },
  { path: '/dispatches', label: '배차 관리', icon: Truck, roles: ['ADMIN', 'DISPATCHER'], mobileVisible: true },
  ...
  { path: '/analytics', label: '통계/분석', icon: BarChart3, roles: ['ADMIN'], mobileVisible: true },
  ...
  { path: '/more', label: '더보기', icon: MoreHorizontal, roles: ['ADMIN', 'DISPATCHER'], mobileVisible: true },
];
...
export const filterMenuByRole = (menuItems: MenuItem[], userRole: string): MenuItem[] => {
  ...
  return menuItems
    ...
    .map(item => {
      if (item.children) {
        return { ...item, children: item.children.filter(child => child.roles.includes(roleUpper)) };
      }
      return item;
    });
};
