...
echo "=== 1. 백업 생성 ==="
cp frontend/src/components/settings/UserManagementTab.tsx frontend/src/components/settings/UserManagementTab.tsx.backup_$(date +%Y%m%d_%H%M%S) || true

echo "=== 2. GitHub에서 최신 파일 다운로드 ==="
curl -L -o frontend/src/components/settings/UserManagementTab.tsx \
  "https://raw.githubusercontent.com/rpaakdi1-spec/3-/main/frontend/src/components/settings/UserManagementTab.tsx"

if [ $? -ne 0 ]; then
  echo "❌ 다운로드 실패. 로컬 파일로 직접 업데이트합니다."
  
  cat > frontend/src/components/settings/UserManagementTab.tsx << 'TSXEOF'
...
echo ""
echo "=== 3. 파일 검증 ==="
ls -lh frontend/src/components/settings/UserManagementTab.tsx
echo ""
echo "파일 주요 내용 확인:"
grep -n "@example.com\|handleToggleActive\|handleEditUser\|showEditModal" frontend/src/components/settings/UserManagementTab.tsx | head -5
...
echo ""
echo "=== 6. 배포 확인 ==="
docker exec uvis-frontend find /usr/share/nginx/html/assets/ -name "SettingsPage*.js" -exec ls -lh {} \;
...
echo ""
echo "📋 업데이트 내용:"
echo "  1. 사용자 활성/비활성 토글 기능 수정 (양방향 전환)"
echo "  2. 사용자 정보 수정 기능 추가 (수정 버튼)"
echo "  3. 사용자 상세 정보 보기 추가"
echo "  4. 이메일 도메인 @example.com으로 수정"
echo ""
echo "🌐 브라우저 테스트:"
echo "  1. Ctrl+Shift+Delete로 캐시 전체 삭제"
echo "  2. 브라우저 완전 종료 후 재시작"
echo "  3. http://139.150.11.99/ 접속"
echo "  4. admin / admin123 로그인"
echo "  5. Settings → 회원관리 탭"
echo "  6. 테스트 수행:"
echo "     - 신규 회원 등록 (이메일 비워두기)"
echo "     - 수정 버튼 클릭하여 정보 수정"
echo "     - 활성/비활성 버튼 클릭하여 상태 토글"
echo ""
echo "End time: $(date)"
